...
| B-09 | Corrupção do pgsi.json no meio da sessão (copiar data/pgsi.json para tmp_path/data/, monkeypatch.chdir, responder q1, sobrescrever a cópia com JSON inválido, resume seguinte levanta PGSIDataError, sem monkeypatch de loader) + teste documentando que dois build_agent() com o mesmo thread_id não compartilham estado (limitação do InMemorySaver, README §10) (F-08, F-09) | tests/test_graph_e2e.py | ::test_pgsi_corruption_mid_session_raises_pgsi_error + ::test_fresh_agent_instance_does_not_share_state | `test: cover mid-session data corruption and persistence limits` | adiantado (decisão de 14/07, P-008): sessão I-010, antes da tag v0.1 |
| B-10 | Resume de thread finalizado/desconhecido (F-05), comportamento já investigado na sessão P-008 (ver achado F-05 acima): finalizado é no-op sem gravar relatório novo; desconhecido levanta KeyError('user_input') e deixa checkpoint parcial pendente, comportamento do langgraph 1.2.9 pinado | tests/test_graph_e2e.py | ::test_resume_on_finished_thread_is_noop + ::test_resume_on_unknown_thread_raises_keyerror | `test: cover resume on finished and unknown threads` | adiantado (decisão de 14/07, P-008): sessão I-010 |
| B-11 | Comportamento de timeout/max_retries com transporte httpx simulado offline (MockTransport injetado via `http_client=` no ChatOpenAI, sem dependência nova): resposta 500 com header retry-after-ms baixo esgota os 2 retries (InternalServerError) e ReadTimeout mapeia para APITimeoutError (F-07) | tests/test_fakes.py | ::test_llm_retries_exhausted_on_server_error + ::test_llm_timeout_maps_to_api_timeout_error | `test: exercise llm timeout behavior under simulated failure` | adiantado (decisão de 14/07, P-008): sessão I-010 |
| B-12 | Stress com o modelo real: 3 sessões e2e consecutivas com respostas mistas dígito/fora-da-tabela (fallback), thread_ids distintos sem vazamento entre threads; resposta na borda de 300 chars chegando ao fallback; timeout contra porta local fechada (sem depender do endpoint vivo) (F-19, F-07). Exige aprovação explícita do usuário antes de chamar o endpoint ativo, mesmo protocolo do O-06/B-08 | tests/test_real_llm.py | ::test_real_llm_stress_sequential_sessions + ::test_real_llm_max_length_answers_hit_fallback + ::test_real_llm_timeout_against_dead_port | `test: add real llm stress scenarios` | adiantado (decisão de 14/07, P-008): sessão I-010, por último (exige endpoint + aprovação) |
| B-13 | Piloto de mutation testing sobre os 3 arquivos completos (parsing.py, safety.py, tools.py), sem amostragem; mutmut via uvx, sem dependência do projeto; limite de execução de ~20 minutos (cortando tools.py primeiro se estourar); roda depois do B-15, para medir a suíte já reforçada pelas propriedades do hypothesis (F-16) | ad hoc via uvx (sem dependência do projeto); relatório versionado em novo Anexo B deste documento | relatório de mutantes gerados/mortos/sobreviventes + triagem de cada sobrevivente (lacuna real vs mutante equivalente); lacunas reais viram itens B-17+ | `docs(audit): record mutation testing pilot results` | adiantado (decisão de 14/07, P-008): sessão I-010, após o B-15 |
| B-14 | Ramos menores do CLI: KeyboardInterrupt (exit 0 + GOODBYE, espelha o teste de EOF existente), RuntimeError de configuração (exit 2), e exceção do classificador propagando sem try/except próprio até o except genérico do CLI (exit 1) (F-10) | tests/test_cli.py | ::test_main_keyboard_interrupt_prints_goodbye + ::test_main_config_error_returns_2 + ::test_main_classifier_error_reaches_generic_handler | `test: cover cli interrupt and config error branches` | adiantado (decisão de 14/07, P-008): sessão I-010 |
| B-15 | **Aprovado em 14/07/2026** (supersede a decisão de 13/07 na pergunta 1 da seção 5, ver pergunta 8): property-based testing com hypothesis (dependência dev nova, perfil determinístico: derandomize=True, max_examples ~200). Propriedades sobre normalize()/parse_answer_deterministic: idempotência de normalize; saída de normalize casa com `^[a-z0-9 ]*$` sem espaços duplos nem bordas; nenhuma das duas funções levanta exceção com texto arbitrário (incluindo Unicode adversarial); toda chave da ANSWER_TABLE decorada com variação aleatória de caixa/espaços/pontuação continua parseando para o mesmo valor; parse sempre devolve um valor em {0,1,2,3,None} | pyproject.toml (grupo dev), tests/test_parsing.py, .gitignore (`.hypothesis/`) | 5 testes `@given` | `test: add property-based tests for parsing` | adiantado (decisão de 14/07, P-008): sessão I-010, antes do B-13 |
