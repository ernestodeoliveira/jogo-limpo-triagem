...
| B-16 | Endurecer a defesa do parser de resposta contra injeção intermitente: temperature=0 no cliente real e/ou reforço do PARSE_SYSTEM_PROMPT combinado com amostragem por maioria (self-consistency: pedir N respostas independentes e só aceitar se a maioria concordar), já que os dois bypasses confirmados devolveram valores dentro da escala válida (3 e 0), não fora dela - um validador de faixa não teria pego nenhum dos dois (F-18). Planejamento detalhado (opções avaliadas, decisão, backlog H-01 a H-07) em `docs/PARSER_HARDENING_PLAN.md`, sessão P-007 | src/triagem/fakes.py, src/triagem/parsing.py | rodar o corpus adversarial do B-08 (tests/test_real_llm.py) N vezes seguidas e medir a taxa de bypass antes/depois da mitigação | `feat(llm): harden answer parser against intermittent prompt injection bypass` | pós-v0.1 |
