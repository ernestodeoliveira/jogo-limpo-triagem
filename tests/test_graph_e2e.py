...
def test_resume_on_finished_thread_is_noop(app, config):
    # F-05: against langgraph resolved to 1.2.9 in uv.lock (pyproject.toml
    # only caps at <2) + InMemorySaver, resuming a thread that already
    # reached finalize is a silent no-op. The checkpoint for a finished
    # thread has no pending task, so invoke() just returns the last saved
    # values again, with no __interrupt__ and without rerunning report_node
    # (no new report file is written to disk). A future langgraph upgrade
    # could change this behavior; re-confirm empirically if this test ever
    # starts failing differently.
    result = app.invoke(initial_state("quero começar o teste"), config)
    for reply in HAPPY_REPLIES:
        result = app.invoke(Command(resume=reply), config)
    ...
    reports_dir = Path(result["report_path"]).parent
...
def test_resume_on_unknown_thread_raises_keyerror(app, config):
    ...
    # through it) has no checkpoint at all. Against langgraph resolved to
    # 1.2.9 in uv.lock (pyproject.toml only caps at <2) + InMemorySaver,
    # this starts a brand new run from START
